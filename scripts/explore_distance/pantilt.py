...
class Pantilt():
    ...
    def set_velocity(self,pan,tilt):
        ...
        if self.pan_vel > MAX_PAN_VEL:
            self.pan_vel = MAX_PAN_VEL
            print 'Max pan velocity is %d' % (MAX_PAN_VEL)
        elif self.pan_vel < MIN_PAN_VEL:
            self.pan_vel = MIN_PAN_VEL
            print 'Min pan velocity is %d' % (MIN_PAN_VEL)
            
        if self.tilt_vel > MAX_TILT_VEL:
            self.tilt_vel = MAX_TILT_VEL
            print 'Max tilt velocity is %d' % (MAX_TILT_VEL)
        elif self.tilt_vel < MIN_TILT_VEL:
            self.tilt_vel = MIN_TILT_VEL
            print 'Min tilt velocity is %d' % (MAX_TILT_VEL)
